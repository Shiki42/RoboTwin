...
class RobotwinRemoteModel:
    ...
    def record_chunk(self, length):
        if not 0 <= length <= self._execution_steps:
            raise ValueError(
                f"client executed {length} actions, server returned {self._execution_steps}"
            )
    ...
    def advance_after_action(self):
        if self._pending_action is None or self._pending_state is None:
            raise RuntimeError("record_action must precede advance_after_action")
        self._client.infer(
            {
                "command": "observe",
