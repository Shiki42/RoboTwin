...
class TimingReceipt:
    ...
    def close(self) -> pathlib.Path:
        ...
        non_checkpoint_rows = [row for row in self._measured if not row.get("checkpoint_saved", False)]
        if len(non_checkpoint_rows) != len(self._measured):
            summary["steady_throughput_without_checkpoint"] = _throughput(non_checkpoint_rows, self._metadata)
            summary["checkpoint_rows"] = len(self._measured) - len(non_checkpoint_rows)
        steady_rows = [row for row in non_checkpoint_rows if not row.get("validation_run", False)]
        if len(steady_rows) != len(non_checkpoint_rows):
            summary["steady_throughput_without_checkpoint_or_validation"] = _throughput(steady_rows, self._metadata)
            summary["validation_rows"] = len(non_checkpoint_rows) - len(steady_rows)
