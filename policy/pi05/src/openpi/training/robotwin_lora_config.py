...
def _create_config(*, lora: bool):
    ...
    return TrainConfig(
        ...
        batch_size=32 if lora else 8,
        gradient_accumulation_steps=1 if lora else 2,
        num_workers=0 if not lora else 2,
        prefetch_factor=2,
        persistent_workers=lora,
        pin_memory=True,
        num_train_steps=30_000,
        save_interval=10_000,
        keep_period=10_000,
        params_only_checkpoint=False,
        ema_decay=0.99 if lora else None,
        seed=42,
        wandb_enabled=False,
        pytorch_compile_mode="default",
        pytorch_attention_implementation="sdpa",
        pytorch_gradient_checkpointing=not lora,
