...
class scan_object_timed(TimedSetup, scan_object):
    def play_once(self):
        ...
        def pickup(arm, actor):
            ...
            if arm == object_arm:
                target = self.right_object_target_pose if arm == 'right' else self.left_object_target_pose
                yield from driver.motion('object_ready', self.place_actor(
                    actor, arm, target, pre_dis=0, dis=0, is_open=False))
